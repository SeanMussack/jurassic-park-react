export const BANNEROFIMAGES = [
    {
        header: 'The Visitor Center has everything your group might need',
        images: [
            {
                src: '/assets/images/thumbnails/visitor-center/visitor-center-thumbnail.png',
                alt: 'The front of the Visitor Center, with trees, trucks, and a pond in front'
            },
            {
                src: '/assets/images/thumbnails/visitor-center/cretaceous-cafe2.png',
                alt: 'Two people sitting at opposite ends of a long table in the Cretaceous Cafe'
            },
            {
                src: '/assets/images/thumbnails/visitor-center/front-door.png',
                alt: 'The front doors of the Visitor Center, surrounded by a fossil relief'
            },
            {
                src: '/assets/images/thumbnails/visitor-center/cretaceous-cafe.png',
                alt: 'Tables and chairs at the Cretaceous Cafe'
            },
            {
                src: '/assets/images/thumbnails/visitor-center/fossil-trex.png',
                alt: 'A fossil of a Tyrannosaurus'
            },
            {
                src: '/assets/images/thumbnails/visitor-center/kitchen.png',
                alt: 'Silver counters and cabinets in our kitchen'
            }
        ]
    },
    {
        header: 'Explore everything the Genetics Lab has to discover',
        images: [
            {
                src: '/assets/images/thumbnails/laboratory/watch-scientists-at-work.png',
                alt: 'Scientists at work, being viewed by visitors through a window'
            },
            {
                src: '/assets/images/thumbnails/laboratory/dna.png',
                alt: 'A DNA molecule on a computer screen'
            },
            {
                src: '/assets/images/thumbnails/laboratory/dr-wu-with-clipboard.png',
                alt: 'Dr. Henry Wu, chief genetic engineer of InGen, writing on a clipboard'
            },
            {
                src: '/assets/images/thumbnails/laboratory/secrets-of-amber.png',
                alt: 'Extracting DNA from a mosquito trapped in amber'
            },
            {
                src: '/assets/images/thumbnails/visitor-center/laboratory.png',
                alt: 'People at work in the laboratory'
            },
            {
                src: '/assets/images/thumbnails/laboratory/hatchery2.png',
                alt: 'A clutch of dinosaur eggs in the hatchery, surrounded by a group of people'
            }
        ]
    },
    {
        header: '',
        images: [
            {
                src: '/assets/images/thumbnails/laboratory/hatchery2.png',
                alt: 'A clutch of dinosaur eggs in the hatchery, surrounded by a group of people'
            },
            {
                src: '/assets/images/thumbnails/visitor-center/showcase-theater.png',
                alt: 'People watching a film at the Showcase Theater'
            },
            {
                src: '/assets/images/thumbnails/laboratory/dr-wu-with-clipboard.png',
                alt: 'Dr. Henry Wu, chief genetic engineer of InGen, writing on a clipboard'
            },
            {
                src: '/assets/images/thumbnails/visitor-center/cretaceous-cafe.png',
                alt: 'Tables and chairs at the Cretaceous Cafe'
            },
            {
                src: '/assets/images/thumbnails/laboratory/secrets-of-amber.png',
                alt: 'Extracting DNA from a mosquito trapped in amber'
            },
            {
                src: '/assets/images/thumbnails/visitor-center/cretaceous-cafe2.png',
                alt: 'Two people sitting at opposite ends of a long table in the Cretaceous Cafe'
            }
        ]
    },
]